...
kill_process() {
    ...
    if [ -n "$pid_watchdog" ]; then
        echo_date "关闭 Clash 看门狗,pid_watchdog:[$pid_watchdog]..."
...
apply_nat_rules() {
    ...
    iptables -t nat -A PREROUTING -p tcp --dport 53 -j koolclash_dns
...
start)
    if [ "$koolclash_enable" == "1" ]; then
        if [ ! -f $KSROOT/koolclash/config/config.yaml ]; then
            ...
            if [ $(yq e '.dns.enable' $KSROOT/koolclash/config/config.yaml) == 'true' ]; then
                start_koolclash
                echo "XU6J03M6"
            else
                echo_date "【没有找到正确的 DNS 配置或 DNS 配置不合法！】"
                echo_date "【Clash 配置文件中 dns.enable 不为 true】"
                echo_date "【KoolClash 将会中断启动并回滚操作！】"
                stop_koolclash
                echo "XU6J03M6"
            fi
